...
func doMap(
	...
	// 2. concrete map function

	mapResult := mapF(inFile, content)
	tempMapResult := make(map[int][]KeyValue)
	for i := 0; i < len(mapResult); i++ {
		item := mapResult[i]
		reduceTaskNumber := ihash(item.Key) % nReduce
		if t, ok := tempMapResult[reduceTaskNumber]; ok {
			t = append(t, item)
			tempMapResult[reduceTaskNumber] = t
		} else {
			t = []KeyValue{}
			t = append(t, item)
			tempMapResult[reduceTaskNumber] = t
	...
	for i := 0; i < nReduce; i++ {
		if t, ok := tempMapResult[i]; ok {
			filename := reduceName(jobName, mapTaskNumber, i)
			writer, err := os.OpenFile(filename, os.O_RDWR|os.O_CREATE, 0666)
			if err != nil {
				log.Fatal("doMap open file: ", err)
			}
