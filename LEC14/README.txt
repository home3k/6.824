6.824 2016 Lecture 14: Spark Case Study
...
Many algorithms are iterative
  ...
  on each iteration, each document:
    ...
    update rank to alpha/N + (1 âˆ’ alpha)*Sum(c_i),
...
One solution: use DSM
  ...
  Bad for fault tolerance
    ...
    expensive in two ways:
      write shared memory to storage during computatoin
...
Core idea in Spark: RDDs
  RDDs are immutable --- you can update them
...
Example use:
  ...
  // now error is materialized in memory
...
Performance
  ...
  In figure 12, k-means on 25 Spark nodes (with no partitions allowed in memory)
    takes 68.8 Difference could be because MapReduce would use replicated
    storage after reduce, but Spark by default would only spill to local disk,
    no network latency and I/O load on replicas.  no architectural reason why
    Spark would be slower than MR
