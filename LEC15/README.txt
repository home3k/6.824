6.824 2017 Lecture 15: Eventual Consistency, Bayou
...
Big points:
  ...
  * Disconnected implies eventual (weak) consistency.
...
Technical ideas to remember:
  ...
  Timestamp, sync, and order to get eventual consistency.
  Conflict resolution via log of write functions, rather than data.
...
Why aren't we satisfied with a central server?
 ...
 Exploit intermittent connectivity to net.
 Exploit intermittent direct contact to other devices.
...
Idea: Bayou's "primary replica" to commit updates.
  ...
  Primary marks each received update with a Commit Sequence Number (CSN).
    ...
    So a complete timestamp is <CSN, local-time, device-id>
...
How Bayou syncs (this is anti-entropy)?
  ...
  Committed updates are easy:
    ...
    A sends log entries between B's CSN and A's CSN (if any)
...
Next: real-world databases with eventual consistency.